...
const duck_nifskorea_com = {
   ...
   */
   readBoard: {
     ...
     * @returns
     */
    pageJSON: function (url, id) {
      var boardData;
      const xhr = new XMLHttpRequest();
      xhr.open("GET", `${duck_nifskorea_com.api_url}/${url}/${id}/`, false);
      xhr.setRequestHeader("Accept", "application/json");
      xhr.send();
    ...
    init_template: function (board) {
      ...
      duck_nifskorea_com.readBoard.init_toast_viewer(data.content);
    },
    /**
     * Initialize markdown viewer for TOAST editor
     * @param {string} contents
     */
    init_toast_viewer(contents) {
      const Viewer = toastui.Viewer;
   ...
   */
    getstudyCardJSON: function () {
      ...
      for (let i = 0; i < Object.keys(data_json).length; i++) {
        var study_data = {
          num: i+1, //순번
          studyName: data_json[i].name, //스터디 이름
          users: data_json[i].users, //사용자
          studyStart: data_json[i].start_date, //업로드 시간
        };
        console.log(study_data);
        duck_nifskorea_com.index.init_study_template(study_data);
      };
    },
      /**
     * 스터디 목록을 template으로 적용
     ...
     */
    init_study_template: function (board) {
      const template = document.getElementById('index_study_template').innerHTML;
      Mustache.parse(template);

      var data = board;
      let rendered = Mustache.render(template, data);
      document.getElementById('index_study_output').innerHTML += rendered;
    },
  },
};
